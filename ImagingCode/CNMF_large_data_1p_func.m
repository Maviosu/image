function CNMF_large_data_1p_func(input_path, gSiz_setting,ring_radius_setting)
%choose data
neuron = Sources2D(); 
nam = [input_path, '\msDownRegist.avi'];          % this demo data is very small, here we just use it as an example
neuron.select_data(nam); 

%% parameters
% -------------------------    COMPUTATION    -------------------------  %
pars_envs = struct('memory_size_to_use', 63, ...   % GB, memory space you allow to use in MATLAB
    'memory_size_per_patch', 9, ...   % GB, space for loading data within one patch
    'patch_dims', [64, 64]);  %GB, patch size

% -------------------------      SPATIAL      -------------------------  %
gSig = 4;           % pixel, gaussian width of a gaussian kernel for filtering the data. 0 means no filtering ;gsig = 1/4*gSiz_setting;forV3 miniscoep:4;v4 miniscope:4;V3downsampleorigin:4
gSiz = gSiz_setting;          % pixel, neuron diameter
ssub = 1;           % spatial downsampling factor ori:1
...
% -------------------------      TEMPORAL     -------------------------  %
Fs = 30/4;             % frame rate ori = 30/4
tsub = 1;           % temporal downsampling factor
deconv_options = struct('type', 'ar1', ... % model of the calcium traces. {'ar1', 'ar2'}
    'method', 'foopsi', ... % method for running deconvolution {'foopsi', 'constrained', 'thresholded'}
    'smin', -8, ...         % minimum spike size. When the value is negative, the actual threshold is abs(smin)*noise level
...
ring_radius = ring_radius_setting;  % when the ring model used, it is the radius of the ring used in the background model.
...
dmin = 10;       % minimum distances between two neurons. it is used together with merge_thr %yz:default:10
dmin_only = 2;  % merge neurons if their distances are smaller than dmin_only.%yz:default:2
